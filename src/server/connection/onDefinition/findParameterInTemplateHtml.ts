...
const findParameterInTemplateHtml = (
  ...
  try {
    ...
    for (const templatePath of possiblePaths) {
      if (fs.existsSync(templatePath)) {
        ...
        if (templateBlockMatch) {
          ...
          const directParamRegex = new RegExp(
            `\\{\\{\\s*${escapedParamName}(?:\\.[a-zA-Z0-9_]+)*\\s*`,
            'g'
          );
          // Pattern 2: {{this.parameterName}} or {{this.parameterName.property}}
          const thisParamRegex = new RegExp(
            `\\{\\{\\s*this\\.${escapedParamName}(?:\\.[a-zA-Z0-9_]+)*\\s*`,
            'g'
          );
          // Pattern 3: {{#if parameterName}}, {{#each parameterName}}, {{#with parameterName}}, etc.
          const blockHelperRegex = new RegExp(
            `\\{\\{#(?:if|unless|each|with)\\s+${escapedParamName}(?:\\.[a-zA-Z0-9_]+)*\\s*`,
            'g'
          );
          // Pattern 4: Function call with parameter: {{helper parameterName}} or {{helper parameterName.property}}
          const helperParamRegex = new RegExp(
            `\\{\\{[a-zA-Z0-9_]+\\s+${escapedParamName}(?:\\.[a-zA-Z0-9_]+)*\\s*`,
            'g'
          );
          // Pattern 5: Subexpressions: (helper parameterName) or (helper parameterName.property)
          const subexpressionRegex = new RegExp(
            `\\([a-zA-Z0-9_]+\\s+${escapedParamName}(?:\\.[a-zA-Z0-9_]+)*`,
            'g'
          );
          ...
          for (const pattern of patterns) {
            ...
            if (
              match &&
              (firstMatchPosition === -1 || match.index < firstMatchPosition)
            ) {
              firstMatch = match;
            ...
            return [
              {
                ...
                range: {
                  start: { line, character: character + paramStartOffset },
                    ...
                    line,
                    character:
                      character + paramStartOffset + parameterName.length,
                  },
                },
              },
            ];
    ...
    console.error(
      `Error finding parameter in template HTML for ${parameterName}:`,
      error
    );
  }
