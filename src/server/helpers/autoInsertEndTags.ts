...
export async function createBlockCompletions(
  ...
  )) as LanguageServerSettings['blockConditions'];
  ...
  const customBlocksWithAutoInsert = customBlocks.filter(
    (block) => block.autoInsertEndTag !== false // Default to true unless explicitly set to false
  );
  ...
  if (trigger.startsWith('complete-')) {
    ...
    const defaultBlock = defaultBlocks.find(
    ...
    const customBlock = customBlocksWithAutoInsert.find(
      (block) => block.type === blockType
    );
    if (customBlock) {
      completions.push({
  ...
  for (const block of defaultBlocks) {
    if (trigger === '#' || block.type.startsWith(trigger.replace('#', ''))) {
      ...
      const label =
        trigger === '#' ? `#${block.type}` : `Complete ${block.type} block`;
 ...
 */
export function shouldProvideBlockCompletion(
  text: string,
  offset: number
): { shouldProvide: boolean; trigger: string } {
      ...
      if (blockMatch) {
        ...
        if (closingTagPattern.test(textAfterBlock)) {
          ...
          if (closingTagMatch) {
            ...
            const closingBlockLine =
              textBeforeClosingTag.split('\n').length - 1;
    ...
    if (blockMatch) {
      ...
      if (nextCloseBrace !== -1) {
        ...
        if (closingTagPattern.test(textAfterCursor)) {
          ...
          if (closingTagMatch) {
            const closingTagOffset =
              offset + nextCloseBrace + 2 + closingTagMatch.index;
            const textBeforeClosingTag = text.substring(0, closingTagOffset);
            const closingBlockLine =
              textBeforeClosingTag.split('\n').length - 1;
  ...
  const blockWithSpaceMatch = beforeCursor.match(
    /\{\{\s*#(if|unless|with|each)\s+$/
  );
